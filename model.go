...
// NOTE: go uses the current locale (eg. +0100 CET) but the createdAt & updatedAt columns are `timestamp without time zone`
// -> when reading the value back into go, it's the exact same hour but the timezone information is missing (+0000 UTC)
// touchCreatedAt & touchUpdatedAt use time.Now() which includes the timezone info, but when saving the value without it then it should correctly be taken without timezone
// ie. otherwise the value is _actually wrong_ (in the future) when read back into GO
func (m *Model) touchCreatedAt() {
	fbn, err := m.fieldByName("CreatedAt")
	if err == nil {
		now := nowFunc().UTC().Truncate(time.Microsecond)
		v := fbn.Interface()
